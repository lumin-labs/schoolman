  ...
  .controller('FinanceCtrl', function ($scope, Forms, Registrar, Fees, Payments) {
   ...
   var stringToNumber = function(amount){
    ...
    Payments.getAll().then(function(paymentsByStudent){
      angular.forEach(paymentsByStudent, function(student, studentId){
        student.studentId = studentId; 
        student.totalPayments = _.reduce(student.payments, function(total, payment){
          return total + payment.amount;
        },0);
      });
      var payments = _.reduce(paymentsByStudent, function(dict, student){
        var form = student.formIndex;
        dict[form] = dict.hasOwnProperty(form) ? dict[form] : {students:[]};
        dict[form].students.push(student);
        return dict;
      },{});

      angular.forEach(payments, function(form, formIndex){
        ...
        form = _.reduce(form.students, function(form, student){
          form.totalDue += student.feeAmount;
          form.totalPaid+= student.totalPayments;
          return form;
        },form);
      });

      console.log("paymentsByStudent", paymentsByStudent);
